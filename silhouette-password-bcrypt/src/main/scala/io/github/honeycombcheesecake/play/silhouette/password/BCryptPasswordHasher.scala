...
import io.github.honeycombcheesecake.play.silhouette.password.BCryptPasswordHasher._
  ...
  override def isDeprecated(passwordInfo: PasswordInfo): Option[Boolean] = isSuitable(passwordInfo) match {
    case suitable if suitable =>
      passwordInfo.password match {
        case LogRoundsPattern(lr) =>
          // Is deprecated if the log rounds has changed
          Some(lr != logRounds.toString)
        case _ =>
          None
      }
    case _ =>
...
object BCryptPasswordHasher {
  ...
  val LogRoundsPattern: Regex = """^\$\w{2}\$(\d{1,2})\$.+""".r
}
