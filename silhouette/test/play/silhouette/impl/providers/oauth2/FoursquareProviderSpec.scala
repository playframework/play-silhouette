...
import play.silhouette.impl.providers.oauth2.FoursquareProvider._
  ...
  "The `retrieveProfile` method" should {
    "fail with ProfileRetrievalException if API returns error" in new WithApplication with Context {
      override def running() = {
        ...
        when(httpLayer.url(eqTo(API.format("my.access.token", DefaultAPIVersion)))).thenReturn(wsRequest)
    ...
    "handle the custom API version property" in new WithApplication with Context {
      override def running() = {
        ...
        when(wsResponse.json).thenReturn(Helper.loadJson("providers/oauth2/foursquare.success.json"))
        when(wsRequest.get()).thenReturn(Future.successful(wsResponse))
        when(httpLayer.url(eqTo(API.format("my.access.token", "20120101")))).thenReturn(wsRequest)
