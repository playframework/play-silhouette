...
/**
 * Test case for the [[io.github.honeycombcheesecake.play.silhouette.api.ErrorHandler]] implementations.
 */
  ...
  "The `DefaultNotAuthenticatedErrorHandler.notAuthenticated` method" should {
    "return an HTML response for an HTML request" in new WithApplication with Context {
      testResponse(
        ...
        f = { notAuthenticated.onNotAuthenticated(_: RequestHeader) })
    }
  ...
  "The `DefaultNotAuthorizedErrorHandler.onNotAuthorized` method" should {
    "return an HTML response for an HTML request" in new WithApplication with Context {
      testResponse(
        ...
        f = { notAuthorized.onNotAuthorized(_: RequestHeader) })
    }
