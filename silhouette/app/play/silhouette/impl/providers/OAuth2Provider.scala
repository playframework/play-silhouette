...
import play.silhouette.impl.providers.state.UserStateItemHandler
...
object OAuth2Info extends OAuth2Constants {
  ...
  /**
   * Converts the JSON into a [[play.silhouette.impl.providers.OAuth2Info]] object.
   */
  implicit val infoReads: Reads[OAuth2Info] = (
    ...
    (__ \ RefreshToken).readNullable[String])((accessToken: String, tokenType: Option[String], expiresIn: Option[Int], refreshToken: Option[String]) =>
...
trait OAuth2Provider extends SocialStateProvider with OAuth2Constants with Logger {
  ...
  def authenticate[S <: SocialStateItem, B](userState: S)(
    ...
    classTag: ClassTag[S]): Future[Either[Result, StatefulAuthInfo[A, S]]] = {
  ...
  def handleFlow[L, R, B](left: => Future[L])(right: String => Future[R])(
    implicit
    request: ExtractableRequest[B]): Future[Either[L, R]] = {
